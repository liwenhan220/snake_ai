...
def train(model, target_model, transition):
    ...
    for state, a, r, is_terminal, next_state in minibatch:
        current_qs = model.predict(np.array(state).reshape(1, *env.observation_space)/255.0)[0]
        if is_terminal:
            new_q = r
        else:
            future_qs = target_model.predict(np.array(next_state).reshape(1, *env.observation_space)/255.0)[0]
            new_q = r + GAMMA * np.max(future_qs)
        current_qs[a] = new_q

        X.append(state)
        y.append(current_qs)
    model.fit(np.array(X).reshape(MINIBATCH_SIZE, *env.observation_space)/255.0, np.array(y), batch_size=MINIBATCH_SIZE, verbose=0)
